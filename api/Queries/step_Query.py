...
def Add_step(item):
    postid = item.postid
    for i in item.steps:
        cursor.execute(f"INSERT INTO \"Step\"(postid,amount,instruction,step_number) VALUES({int(postid)},'{i.amount}','{i.instruction}','{i.step_number}')")
...
def Edit_step(item):
    ...
    for i in item.steps:
        cursor.execute(f"UPDATE \"Step\" SET \"amount\" = '{i.amount}' , \"instruction\" = '{i.instruction}'  WHERE \"step_number\" = '{i.step_number}'")
...
def Delete_step(PID,SN):
    cursor.execute(f"DELETE FROM \"Step\" WHERE \"postid\" = {PID} AND \"step_number\" = '{SN}'")
    conn.commit()
