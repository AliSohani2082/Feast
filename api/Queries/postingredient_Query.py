...
def All_ingredient(PID):
    cursor.execute(
        f"SELECT \"Post_ingredient\".*,\"ingredient\".\"name\" FROM \"Post_ingredient\" INNER JOIN  \"ingredient\" ON \"Post_ingredient\".\"ingredientid\" = \"ingredient\".\"ID\" WHERE \"postid\" = {PID}")
...
def Add_ingredient(item):
    ...
    for i in item.ingredient:
        cursor.execute(
            f"INSERT INTO \"Post_ingredient\"(postid,ingredientid,amount) VALUES({postid},{i.ingredientid},'{i.amount}')")
...
def Delete_ingredient(PID, IID):
    ...
    cursor.execute(
        f"DELETE FROM \"Post_ingredient\" WHERE \"postid\" = {PID} AND \"ingredientid\" = {IID}")
