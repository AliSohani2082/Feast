...
def All_post():
    ...
    cursor.execute(
        "SELECT \"ID\",\"userid\",\"title\",\"description\",\"image\",\"like_count\" FROM \"Post\"")
...
def Specific_post(PID):
    ...
    cursor.execute(
        f"SELECT \"ID\",\"userid\",\"title\",\"description\",\"image\",\"like_count\" FROM \"Post\" WHERE \"ID\" = {int(PID)}")
...
def Add_post(userid, TITLE, DESCRIPTION, IMAGE, INGREDIENT, STEPS):
    cursor.execute(
        f"INSERT INTO \"Post\"(userid,title,description,image,like_count,time) VALUES({int(userid)},'{TITLE}','{DESCRIPTION}','{IMAGE}',0,CURRENT_DATE) RETURNING \"ID\";")
    ...
    for i in STEPS:
        cursor.execute(
            f"INSERT INTO \"Step\"(postid,instruction,step_number) VALUES({int(id[0])},'{i}','{step_number}')")
        step_number += 1
...
def Reaction(PID, UID, mode):
    if mode == 'like':
        cursor.execute(
            f"UPDATE \"Post\" set \"like_count\" = \"like_count\ + 1")
        conn.commit()
        return {"status_code": 202, "content": "The post was liked"}
    elif mode == 'dislike':

        cursor.execute(
            f"UPDATE \"Post\" set \"like_count\" = \"like_count\ - 1 WHERE \"like_count\" <> 0")
        conn.commit()
...
def Add_posts(item):
    ...
    for i in item:
        cursor.execute(
            f"INSERT INTO \"Post\"(userid,title,description,image,like_count,time) VALUES({int(i.userid)},'{i.title}','{i.description}','{i.image}',0,CURRENT_DATE) RETURNING \"ID\";")
