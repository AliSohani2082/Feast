...
def Add_post(userid, TITLE, DESCRIPTION, IMAGE, INGREDIENT, STEPS):
    ...
    for i in STEPS:
        cursor.execute(
            f"INSERT INTO \"Step\"(postid,amount,instruction,step_number) VALUES({int(id[0])},'{i.amount}','{i.instruction}','{i.step_number}')")

    for i in INGREDIENT:
        cursor.execute(
            f"INSERT INTO \"Post_ingredient\"(postid,amount,ingredientid) VALUES({int(id[0])},'{i.amount}','{i.ingredientid}')")
...
def Edit_post(PID, title, description, image, amount):
    cursor.execute(
        f"UPDATE \"Post\" set \"title\" = {title} , \"description\" = {description} , \"image\" = {image} , \"amount\" = {amount} where \"ID\" = {int(PID)}")
    cursor.commit()
