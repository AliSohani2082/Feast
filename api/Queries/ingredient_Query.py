...
def Add_ingredient(item):
    ...
    for i in item:
        print(i)
        cursor.execute(
            f"SELECT * FROM \"ingredient\" WHERE \"name\"='{i.name}'")
        ...
        if result:
            ...
            print(i.name)
            cursor.execute(
                f"INSERT INTO \"ingredient\"(name,type,price_per_unit,image,unit_type) VALUES('{i.name}','{i.type}','{i.price_per_unit}','{i.image}','{i.unit_type}')")
            conn.commit()
